...
function fntUpdateCant(pro,cant){
	if(cant <= 0){
	    ...
	    request.onreadystatechange = function(){
	    	...
	    	if(request.status == 200){
	    		let objData = JSON.parse(request.responseText);
	    		if(objData.status){         //se le pone [0] porque se refiere a una clase
...
//cuando el pago de la compra no sea con paypal
if (document.querySelector("#btmComprar")){
	let btnPago = document.querySelector("#btmComprar");
