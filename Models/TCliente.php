...
trait TCliente
{
	...
	public function insertDetalleTemp(array $pedido)
	{
		...
		//verifica si existen productos en el carrito
		$this->con = new Mysql();
		$sql = "SELECT * FROM detalle_temp WHERE
				 transaccionid = '{$this->intIdTransaccion}' AND personaid = $this->intIdUsuario";
		$request = $this->con->select_all($sql);
		...
		{
			//si existen productos en el carrito se actualiza la tabla con los nuevos productos
			$sqlDel = "DELETE FROM detalle_temp WHERE transaccionid = '{$this->intIdTransaccion}'
	...
	{
		$this->con = new Mysql();
		//$this->intIdTransaccion = $idtransaccionpaypal;
